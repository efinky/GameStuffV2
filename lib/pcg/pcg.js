...
export class PCG32 {
  ...
  advance(delta) {
    if (delta < 0) {
      ...
      delta = MAX_UINT64 + (delta & (MAX_UINT64)-1n);
    }
  ...
  /** @param {{ state: string; inc: string }} param */
  static fromJSON({ state, inc }) {
    return Object.setPrototypeOf({state: BigInt(state), inc: BigInt(inc)}, PCG32.prototype);
  }
