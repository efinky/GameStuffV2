...
export class Server {
  ...
  constructor(callbacks, now = performance.now()) {
    ...
    this.stateAtLastConnect = JSON.stringify(this.callbacks.getState());
    /** @type {any[]} */
  ...
  broadcast(msg) {
    const data = serializer.stringify(msg);
  ...
  async onConnect(channel) {
    ...
    channel.onmessage = (e) => {
      ...
      const peerEvent = serializer.parse(e.data);
    ...
    this.stateAtLastConnect = JSON.stringify(state);
    this.eventsSinceLastConnect = [];
