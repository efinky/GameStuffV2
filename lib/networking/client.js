...
import { serializer } from "../../worldState.js";
...
export class Client {
  ...
  constructor({
    ...
    this.stateAtLastConnect = "";
    /** @type {any[]} */
    this.eventsSinceLastConnect = [];
  }

  /** @param {E} peerEvent */
  sendEvent(peerEvent) {
    const msg = serializer.stringify(peerEvent);
    this.channel.send(msg);
  }

  getEvents(time = performance.now()) {
    const events = this.eventQueue.getEvents(time);
    this.eventsSinceLastConnect.push(events);
    return events;
  }
  ...
  static async init(channel) {
    ...
    client.stateAtLastConnect = JSON.stringify(state);
    /** @type {any[]} */
    client.eventsSinceLastConnect = [];

    channel.onmessage = (e) => {
      /** @type {Message<S, E>} */
      const msg = serializer.parse(e.data);
